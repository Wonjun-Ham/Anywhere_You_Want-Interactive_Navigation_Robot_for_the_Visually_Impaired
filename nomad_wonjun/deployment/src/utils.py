...
# process_data_utils  process_locobot_img, custom image processing functions 부분 참고.
# ---------------------------------------------------------
...
# 추가. CompressedImage일 때. 파이썬은 오버로딩 안 되므로 같은 함수로 같이 넣어줘야  
# ---------------------------------------------------------
...
def transform_images(pil_imgs: List[PILImage.Image], image_size: List[int], center_crop: bool = False) -> torch.Tensor:
    ...
    # C, 즉 rgb채널 dimension. 이게 3개가 아닌, 3*image 갯수 만큼이 되는 것
