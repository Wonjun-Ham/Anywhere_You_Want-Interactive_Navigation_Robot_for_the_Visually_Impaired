...
def train_nomad(
    ...
    # train eval loop nomad, train.py로 거슬러 올라가면 
    # train_loader = DataLoader(train_dataset,...)으로 정의돼있는 것이 여기 온 것
    # train_dataset에 대한 코드를 모아보면,
    # train_dataset = []
    # if data_split_type == "train": train_dataset.append(dataset)
    # train_dataset = ConcatDataset(train_dataset)
    # 이때 dataset은 dataset = ViNT_Dataset(...,
    # image_size: Tuple[int, int],
    # waypoint_spacing: int,
    # min_dist_cat: int,
    # max_dist_cat: int,
    # min_action_distance: int,
    # max_action_distance: int,...)로 정의됨
    
    # 아래에서 data는 vint_dataset.py에서 __getitem__ 통해 가져옴
    with tqdm.tqdm(dataloader, desc="Train Batch", leave=False) as tepoch:
        for i, data in enumerate(tepoch):
            ...
            # unnormalize와 cumsum의 역과정 여기서 진행
...
def get_action(diffusion_output, action_stats=ACTION_STATS):
    ...
    # def unnormalize_data(ndata, stats):
    # First: (ndata + 1) / 2 converts data from [-1, 1] range to [0, 1] range
    # ndata = (ndata + 1) / 2
    # data = ndata * (stats['max'] - stats['min']) + stats['min']
    # return data
    actions = np.cumsum(ndeltas, axis=1)
    # delta 값들을 시간별 absolute position으로
    # [[[1,0], [2,1], [1,2]]] -> [[[1,0], [3,1], [4,3]]]
