...
# train.py에서 쓰임. 아래 여러 함수들 중 밖에서 쓰이는 건 len, getitem 매직메서드 뿐
# 나머진 그걸 위한 것
...
# 아래 두 함수는 line 220 225 281에 pickle.loads,dumps에서 자동 호출됨
    ...
    def _load_image(self, trajectory_name, time):
        ...
        try:
            ...
            # data_utils의 resize_and_aspect_crop 실행
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        ...
        """
        # _build_index()에서 만들어진 걸로 그냥 traj_name, curr_time, max_goal_distance 들어있는데,max_goal_distance는 쓰지도 않는 것
        # 첫번째 trajecotry부터 모든 가능한 경우들 다 고려해서 튜플 만들어둔 것일 뿐 
        f_curr, curr_time, max_goal_dist = self.index_to_data[i]
        # 주변 말고 먼 경우들, 심지어 negative goal이라는 다른 경로의 매우 먼 경우에 대한 시간 계산도 학습시켜 다양한 걸로 학습시키는 것
